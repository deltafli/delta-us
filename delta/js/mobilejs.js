 // Add interactive functionality
        ...
        document.addEventListener('DOMContentLoaded', function () {
  // Select all date wrappers
  var wrappers = document.querySelectorAll('.q7f3s_date_wrapper');

  function updateWrapperState(wrapper) {
    var input = wrapper.querySelector('.q7f3s_date_input');
    if (!input) return;
  ...
  wrappers.forEach(function (wrapper) {
    var input = wrapper.querySelector('.q7f3s_date_input');
    var placeholderSpan = wrapper.querySelector('.q7f3s_date_placeholder');

    // If you want the placeholder text to come from data-placeholder attr:
    if (placeholderSpan && wrapper.dataset.placeholder) {
      placeholderSpan.textContent = wrapper.dataset.placeholder;
    }

    // Initialize state (in case there's a pre-filled value)
    updateWrapperState(wrapper);

    // On input/change update the wrapper class so placeholder hides/shows
    ...
    // Optional: when clicking placeholder focus the input (nice UX)
    if (placeholderSpan) {
      placeholderSpan.addEventListener('click', function () {
        input.focus();
      });
    }
  });
