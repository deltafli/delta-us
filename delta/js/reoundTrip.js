...
function toggleExperienceSection() {
   ...
   function selectExperience(type, price) {
     ...
     window.location.href = 'one-way-back.html?' + params.toString();
   }
 ...
 function setupDelegatedOpener(){
  ...
  const handler = function(e){
    ...
    if(isMobile){
      // MODIFICACIÓN: Ahora cualquier parte de la tarjeta de vuelo es clickeable
      const card = e.target.closest('.flight-info-card');
      if(!card) return;
      
      section = card.closest('section.flight-details-section');
      
      // Prevenir acción por defecto y propagación
      e.preventDefault();
      e.stopPropagation();
    } else {
      // Comportamiento para desktop (mantener existente)
